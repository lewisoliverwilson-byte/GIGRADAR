import React from 'react';
import Link from 'next/link';

export default function Footer() {
  return (
    <footer className="border-t border-white/5 bg-surface-1 mt-20">
      <div className="section py-12">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-8 mb-10">
          <div className="col-span-2 sm:col-span-1">
            <Link href="/" className="flex items-center gap-2 mb-3">
              <span className="text-xl font-black tracking-tight">
                <span className="text-brand">Gig</span><span className="text-white">Radar</span>
              </span>
            </Link>
            <p className="text-sm text-zinc-500 leading-relaxed">
              Every UK gig, one place. Updated every 6 hours from 10+ ticket sources.
            </p>
          </div>
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest text-zinc-500 mb-3">Discover</p>
            <ul className="space-y-2">
              {[['Gigs', '/gigs'], ['Artists', '/artists'], ['Venues', '/venues'], ['Discover', '/discover']].map(([label, href]) => (
                <li key={href}><Link href={href} className="text-sm text-zinc-400 hover:text-white transition-colors">{label}</Link></li>
              ))}
            </ul>
          </div>
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest text-zinc-500 mb-3">Account</p>
            <ul className="space-y-2">
              {[['Sign up', '/'], ['Log in', '/'], ['Profile', '/profile']].map(([label, href]) => (
                <li key={label}><Link href={href} className="text-sm text-zinc-400 hover:text-white transition-colors">{label}</Link></li>
              ))}
            </ul>
          </div>
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest text-zinc-500 mb-3">Sources</p>
            <ul className="space-y-2">
              {['Ticketmaster', 'Dice', 'Skiddle', 'Songkick', 'See Tickets', 'Bandsintown'].map(s => (
                <li key={s}><span className="text-sm text-zinc-500">{s}</span></li>
              ))}
            </ul>
          </div>
        </div>
        <div className="divider pt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-xs text-zinc-600">© {new Date().getFullYear()} GigRadar. Not affiliated with any ticket vendor.</p>
          <p className="text-xs text-zinc-600">Data updated every 6 hours · 18,000+ artists · 4,700+ venues</p>
        </div>
      </div>
    </footer>
  );
}
