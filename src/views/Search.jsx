...
export default function Search() {
  ...
  return (
    <>
      <Head>
        <title>{query ? `"${query}" — GigRadar Search` : 'Search — GigRadar'}</title>
      </Head>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-8">
        <div className="relative">
          ...
          />
          {query && (
            <button onClick={() => { setQuery(''); router.replace('/search', undefined, { shallow: true }); }}
        ...
        {loading ? (
          ...
          <div className="text-center text-gray-500 text-sm py-8">Start typing to search artists and venues.</div>
        ) : !hasResults ? (
          <div className="card p-10 text-center text-gray-500 text-sm">
            <p>No results for <span className="text-white">"{query}"</span></p>
            <p className="mt-2 text-xs">If they're a UK act, they may appear soon as we scan more venues.</p>
            <Link href="/discover" className="mt-4 inline-block text-brand hover:underline text-sm">Browse all gigs →</Link>
          </div>
        ) : (
          <>
            {matchedArtists.length > 0 && (
              <section>
                <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">Artists ({matchedArtists.length})</h2>
            ...
            {matchedVenues.length > 0 && (
              <section>
                <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">Venues ({matchedVenues.length})</h2>
